...
namespace Geldautomat
{
    public class PrinterConfigForm : Form
    {
        ...
        private CheckBox chkShowWorkTimesOnly, chkShowPauseTime, chkAutoPauseDeduction, chkShowArbeitszeit;
        ...
        private void BuildWindowsTab(TabPage parent){ var pnl = new Panel { Dock = DockStyle.Fill, Padding = new Padding(24,12,24,12), AutoScroll = true, BackColor = Color.White }; parent.Controls.Add(pnl); int y=8; chkEnabled = MakeCheck(pnl, "Druck aktivieren", ref y); chkAsk = MakeCheck(pnl, "Vor Druck fragen", ref y); chkVat = MakeCheck(pnl, "MwSt.-Zeilen drucken", ref y); chkCut = MakeCheck(pnl, "Nach Druck schneiden", ref y); chkAutoIfNoPrompt = MakeCheck(pnl, "Automatisch drucken wenn keine Nachfrage", ref y);
    // Arbeits-/Pausen-Optionen
    chkShowWorkTimesOnly = MakeCheck(pnl, "Arbeitszeiten einblenden (Anfang/Ende)", ref y);
    chkShowPauseTime = MakeCheck(pnl, "Pausenzeiten anzeigen", ref y);
    chkAutoPauseDeduction = MakeCheck(pnl, "Automatischer Pausenabzug (30/45 Min)", ref y);
    chkShowArbeitszeit = MakeCheck(pnl, "Arbeitszeit anzeigen", ref y);
    MakeLabel(pnl, "Primärer Drucker:", ref y); cboPrinters = new ComboBox { Location=new Point(260,y-30), Size=new Size(320,28), DropDownStyle=ComboBoxStyle.DropDownList }; pnl.Controls.Add(cboPrinters); y+=10; chkUseSecondary = MakeCheck(pnl, "Sekund�ren Drucker bei Fehler verwenden", ref y); MakeLabel(pnl, "Sekundärer Drucker:", ref y); cboPrinters2 = new ComboBox { Location=new Point(260,y-30), Size=new Size(320,28), DropDownStyle=ComboBoxStyle.DropDownList }; pnl.Controls.Add(cboPrinters2); y+=10; MakeLabel(pnl, "Zeichen / Zeile:", ref y); nudChars = new NumericUpDown { Location=new Point(260,y-30), Minimum=10, Maximum=120, Value=40, Size=new Size(80,28) }; pnl.Controls.Add(nudChars); y+=20; int btnY = y+10; btnSave = MakeActionButton(pnl, "Speichern", new Point(260,btnY), Color.FromArgb(46,125,50), (s,e)=>SaveAndClose()); btnTest = MakeActionButton(pnl, "Testdruck", new Point(380,btnY), Color.FromArgb(33,150,243), (s,e)=>DoTestPrint()); btnCancel = MakeActionButton(pnl, "Abbrechen", new Point(500,btnY), Color.FromArgb(120,120,120), (s,e)=>Close()); }
