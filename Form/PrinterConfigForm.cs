...
namespace Geldautomat
{
    public class PrinterConfigForm : Form
    {
        ...
        private void SaveDocPrinter(){ try { var name = cboDocPrinters.SelectedItem as string ?? string.Empty; IniHelper.WriteValue("UI", "DocumentPrinter", name, AppSettings.IniPath); MessageBox.Show(this, "Dokumentendrucker gespeichert.", "Info", MessageBoxButtons.OK, MessageBoxIcon.Information); } catch (Exception ex) { MessageBox.Show(this, "Speichern fehlgeschlagen:\r\n" + ex.Message, "Fehler", MessageBoxButtons.OK, MessageBoxIcon.Error); } }
        private void TestDocPrinter(){ try { var name = cboDocPrinters.SelectedItem as string ?? string.Empty; if (string.IsNullOrWhiteSpace(name)) { MessageBox.Show(this, "Bitte Drucker auswählen.", "Hinweis", MessageBoxButtons.OK, MessageBoxIcon.Information); return; } string tmp = System.IO.Path.GetTempFileName(); System.IO.File.WriteAllText(tmp, "Test Dokumentendrucker"); var psi = new System.Diagnostics.ProcessStartInfo { FileName = tmp, Verb = "printto", Arguments = '"' + name + '"', UseShellExecute = true, CreateNoWindow = true, WindowStyle = System.Diagnostics.ProcessWindowStyle.Hidden }; System.Diagnostics.Process.Start(psi); MessageBox.Show(this, "Testdruck gesendet.", "Info", MessageBoxButtons.OK, MessageBoxIcon.Information); } catch (Exception ex) { MessageBox.Show(this, "Testdruck fehlgeschlagen:\r\n" + ex.Message, "Fehler", MessageBoxButtons.OK, MessageBoxIcon.Error); } }
    }
}
