...
namespace Geldautomat
{
    public class PrinterConfigForm : Form
    {
        ...
        private Panel _customTabHeader; private Button _btnTabWin; private Button _btnTabLegacy;
        ...
        {
            FormBorderStyle = FormBorderStyle.None; StartPosition = FormStartPosition.CenterScreen; DoubleBuffered = true; BackColor = Color.White; Font = new Font("Segoe UI", 10F); ClientSize = new Size(760, 560);
            ...
            Resize += (s,e)=> AdjustLayout(); // ensure layout on resize
            AdjustLayout();
        }
        ...
        private void ForceCustomTabHeader()
        {
            try
            {
                ...
                // Insert in correct order: header (index 0), custom header (index 1), tabs (index 2)
                ...
                _customTabHeader.Controls.Add(_btnTabWin); _customTabHeader.Controls.Add(_btnTabLegacy);
        ...
        private void UpdateCustomTabButtons(){ if (_btnTabWin == null || _btnTabLegacy == null || _tabs == null) return; bool win = _tabs.SelectedIndex==0; ApplyTabButtonStyle(_btnTabWin, win); ApplyTabButtonStyle(_btnTabLegacy, !win); }
        ...
        private void SaveAndClose(){ try { _settings.Enabled=chkEnabled.Checked; _settings.AskUser=chkAsk.Checked; _settings.PrintVatLines=chkVat.Checked; _settings.CutAfterPrint=chkCut.Checked; _settings.AutoPrintIfNoPrompt=chkAutoIfNoPrompt.Checked; _settings.CharsPerLine=(int)nudChars.Value; _settings.PrinterName=cboPrinters.SelectedItem as string ?? string.Empty; _settings.SecondaryPrinterName=cboPrinters2.SelectedItem as string ?? string.Empty; _settings.UseSecondaryOnFailure=chkUseSecondary.Checked; _settings.Legacy1Enabled=chkL1Enabled.Checked; _settings.Legacy1ComPort=cboL1Port.SelectedItem as string ?? string.Empty; _settings.Legacy1Baud=(int)nudL1Baud.Value; _settings.Legacy1Chars=(int)nudL1Chars.Value; _settings.Legacy1Cut=chkL1Cut.Checked; _settings.Legacy2Enabled=chkL2Enabled.Checked; _settings.Legacy2ComPort=cboL2Port.SelectedItem as string ?? string.Empty; _settings.Legacy2Baud=(int)nudL2Baud.Value; _settings.Legacy2Chars=(int)nudL2Chars.Value; _settings.Legacy2Cut=chkL2Cut.Checked; _settings.Save(); DialogResult=DialogResult.OK; Close(); } catch (Exception ex) { MessageBox.Show(this, "Fehler beim Speichern:\r\n"+ex.Message, "Fehler", MessageBoxButtons.OK, MessageBoxIcon.Error); } }

        // Build temporary settings from current UI (unsaved) for testing
        private ReceiptPrinterSettings BuildCurrentSettings()
        {
            return new ReceiptPrinterSettings
            {
                Enabled = chkEnabled.Checked,
                AskUser = chkAsk.Checked,
                PrintVatLines = chkVat.Checked,
                CutAfterPrint = chkCut.Checked,
                AutoPrintIfNoPrompt = chkAutoIfNoPrompt.Checked,
                CharsPerLine = (int)nudChars.Value,
                PrinterName = cboPrinters.SelectedItem as string ?? string.Empty,
                SecondaryPrinterName = cboPrinters2.SelectedItem as string ?? string.Empty,
                UseSecondaryOnFailure = chkUseSecondary.Checked,
                Legacy1Enabled = chkL1Enabled.Checked,
                Legacy1ComPort = cboL1Port.SelectedItem as string ?? string.Empty,
                Legacy1Baud = (int)nudL1Baud.Value,
                Legacy1Chars = (int)nudL1Chars.Value,
                Legacy1Cut = chkL1Cut.Checked,
                Legacy2Enabled = chkL2Enabled.Checked,
                Legacy2ComPort = cboL2Port.SelectedItem as string ?? string.Empty,
                Legacy2Baud = (int)nudL2Baud.Value,
                Legacy2Chars = (int)nudL2Chars.Value,
                Legacy2Cut = chkL2Cut.Checked
            };
        }

        private void DoTestPrint(){ try { var cfg = BuildCurrentSettings(); var lines = new[]{"Testdruck","Primär: "+(cfg.PrinterName??"(Standard)"),"Sekundär: "+(string.IsNullOrWhiteSpace(cfg.SecondaryPrinterName)?"(keiner)":cfg.SecondaryPrinterName),"Fallback aktiv: "+(cfg.UseSecondaryOnFailure?"Ja":"Nein"),"Zeit: "+DateTime.Now.ToString("dd.MM.yyyy HH:mm:ss"),"Zeichen/Zeile: "+cfg.CharsPerLine,"MwSt.-Zeilen: "+(cfg.PrintVatLines?"Ja":"Nein")}; new ReceiptPrinter(cfg).PrintSimpleReceipt("TEST", lines); MessageBox.Show(this,"Testdruck gesendet (unsaved settings).","Info",MessageBoxButtons.OK,MessageBoxIcon.Information);} catch(Exception ex){ MessageBox.Show(this,"Testdruck fehlgeschlagen:\r\n"+ex.Message,"Fehler",MessageBoxButtons.OK,MessageBoxIcon.Error);} }
