...
namespace TaMi_Einzahlautomat
{
    public class KassensturzSmartCoinForm : Form
    {
        ...
        private Label _lblSnapshotInfo; // Hinweis f�r aktiven Snapshot
        ...
        private SmartCoinV1 GetSmartCoinInstance()
        {
            ...
            if (CoinManager.Instance is SmartCoinV1 sc1)
                return sc1;
            // Fallback: eigene Instanz, falls CoinManager.Instance nicht verf�gbar
            return smartCoin;
        }
        ...
        private async Task RefreshBestandCoinsAsync()
        {
            ...
            try
            {
                ...
                // NEU: Wenn kein Snapshot aktiv ist (nach 'Alle M�nzen eingezahlt'),
        ...
        private void UpdateCoinsUiFromLevels(int[] lv)
        {
            ...
            for (int i = 0; i < 8; i++)
            {
                ...
                if (lblRechtsBestand[i] != null)
                {
                    if (v == -2)
                    {
                        // Keine �nderung, Wert bleibt wie er ist
                    }
                    else if (v < 0)
                    {
        ...
        private void UpdateDifferenz()
        {
            ...
            string text = "Differenz: " + diff.ToString("N2") + " €"; // N2 + Euro-Zeichen f�r konsistente Gruppierung
            ...
            if (_snapshotLoaded)
            {
                ...
                if (res == DialogResult.No)
                {
                    ...
                    try
                    {
                        ...
                        initialSumme = BerechneSumme(initialBestand);
                        ZeigeInitialBestand();
                        SaveSnapshot(live); // �berschreibt alten
                        ShowSnapshotInfo();
        ...
        private async Task CountdownAndSmartEmpty()
        {
            using (var countdownForm = new Form())
            {
                ...
                countdownForm.Show(this); // Modalit�t vermeiden, UI bleibt reaktionsf�hig
                for (int i = 3; i >= 1; i--)
                {
                    lbl.Text = i.ToString();
            ...
            try
            {
                ...
                _tmrBestand.Enabled = false; // w�hrend SmartEmpty keine Poll-Requests
                await Task.Run(() =>
                {
            ...
            {
                await Task.Delay(1500); // etwas warten bis Ger�t Levels aktualisiert
                _tmrBestand.Enabled = true;
                RefreshBestandCoins();
        ...
        // Fallback Oberfl�che bei fehlender SmartCoin-Initialisierung
