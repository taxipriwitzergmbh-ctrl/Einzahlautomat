...
namespace TaMi_Einzahlautomat
{
    public class KassensturzRm5Form : Form
    {
        ...
        public KassensturzRm5Form(Rm5CctalkValidator rm5)
        {
            ...
            StartAutoRefresh();
            AttachRm5Events();
            // Tastatursteuerung (für Pfeiltasten zuverläsiger über ProcessCmdKey)
        ...
        private void BuildLayout()
        {
            ...
            try { ApplyModernButtonStyle(btnEntleeren, Color.FromArgb(33, 150, 243), Color.FromArgb(13, 71, 161)); } catch { }
            btnEntleeren.Click += BtnEntleeren_Click;
            Controls.Add(btnEntleeren);

            // NEU: Einzel-Entleerung je M�nzsorte (10c..2�)
            ...
            for (int i = 3; i < 8; i++)
            {
                ...
                b.FlatAppearance.BorderSize = 0;
                try { ApplyModernButtonStyle(b, Color.FromArgb(96, 125, 139), Color.FromArgb(55, 71, 79)); } catch { }
            ...
            _btnManualAdjust.FlatAppearance.BorderSize = 0;
            try { ApplyModernButtonStyle(_btnManualAdjust, Color.FromArgb(96, 125, 139), Color.FromArgb(55, 71, 79)); } catch { }
            _btnManualAdjust.Click += (s, e) => ToggleManualAdjust();
            Controls.Add(_btnManualAdjust);
            ...
            try { ApplyModernButtonStyle(btnAlleEingezahlt, Color.FromArgb(46, 125, 50), Color.FromArgb(27, 94, 32)); } catch { }
            btnAlleEingezahlt.Click += BtnAlleEingezahlt_Click;
        ...
        private void BtnEmptySingle_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                // Initialen Level optional erfassen (nur f�r 2. Lauf-Entscheidung), aber nicht blockieren
                int level = -1;
                ...
                b.Enabled = false;
                if (btnEntleeren != null) btnEntleeren.Enabled = false; // w�hrend Einzelentleerung globalen Start sperren
        ...
        {
            // Benutzer best�tigt, dass alle vorhandenen M�nzen als eingezahlt gelten
            try
            {
                ...
                // WICHTIG: Beim RM5 den rechten (aktuellen) Bestand als Ger�te-/Referenzbestand �bernehmen
                try { _rm5?.SetAllCoinLevels(cur, true); } catch { }
                try { AppLogger.Log("[RM5] Manuell: Alle M�nzen eingezahlt -> SetAllCoinLevels(current, persist)"); } catch { }
                ...
                // UI aktualisieren: Links = Referenz (neu), Rechts = aktuell (unver�ndert)
                ShowInitial();
                HideSnapshotInfo();
                btnEntleeren.Enabled = true;
            ...
            {
                MessageBox.Show(this, "Fehler beim �bernehmen des aktuellen Bestands: " + ex.Message, "Fehler", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
        ...
        private void AdjustSelectedLevel(int delta)
        {
            try
            {
                if (_emptyStarted || _singleDrainInProgress || _uiZeroLocked) return; // w�hrend Entleerung gesperrt
            ...
            try
            {
                for (int i = 3; i < 8; i++)
                {
                    ...
                    if (_btnMinus[i] == null)
                    {
                        ...
                        try { btn.Region = Region.FromHrgn(CreateRoundRectRgn(0, 0, btn.Width, btn.Height, 8, 8)); } catch { }
