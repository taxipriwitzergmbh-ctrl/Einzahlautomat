...
namespace TaMi_Einzahlautomat
{
    public class KassensturzNV200 : Form
    {
        ...
        // Merker f�r Startlog (nur einmal)
        private bool _startLogged = false;

        // Laufzeitdarstellung w�hrend Auszahlung
        private bool _dispensingActive = false;
        private int[] _dispenseViewCounts = null; // gespiegelt/erwartete Payout-Best�nde (w�hrend Auszahlung)
        ...
        {
            if (_ssp == null) return;
            // Kassensturz aktiv: Personalguthaben darf sich nicht �ndern
            try { AppLogger.KassensturzScopeEnter(); } catch { }

            // Snapshot / Startlogik unver�ndert
            if (_snapshotLoaded)
            {
                var res = MessageBox.Show(this,
                    "Es l�uft noch ein aktiver Kassensturz.\nJa = fortfahren | Nein = Neuer Start | Abbrechen = Abbruch",
                    "Aktiver Kassensturz",
            ...
            // WICHTIG: MessageBox jetzt VOR dem Start der Auszahlung (nicht blockierend w�hrend Events)
            ...
            // Auszahlung starten (UI jetzt frei f�r Events/Timer)
            ...
            // Sofort ein erstes Refresh ansto�en (falls interne Werte schon 0 gesetzt wurden)
            RefreshPayout();
        ...
        private void RefreshPayout()
        {
            ...
            // W�hrend einer Auszahlung keine Level-Requests senden (w�rden ohnehin �bersprungen)
        ...
        private void ClearSnapshot() { try { IniHelper.WriteValue(SnapshotSection, _snapshotKey, string.Empty, AppSettings.IniPath); } catch { } try { AppLogger.Log("[NV200] Snapshot gel�scht"); } catch { } }
