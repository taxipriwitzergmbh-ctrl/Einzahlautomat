...
namespace Geldautomat.Coins
{
    public static class Coin2Manager
    {
        ...
        public static ICoinValidator Instance { get; set; }
        ...
        private static void Log(string msg) { try { AppLogger.Log("[Coin2] " + msg); } catch { } }
            ...
            lock (_lock)
            {
                ...
                try
                {
                    if (Instance is SmartCoinV1 sc1)
                    {
                        ...
                        try { sc1.SetStatus("konfig deaktiviert"); } catch { }
                    }
                    else
                    {
        ...
        public static void EnableForConfig(string iniPath = null)
        {
            lock (_lock)
            {
                ...
                try { InitFromIni(iniPath); } catch (Exception ex) { Log("EnableForConfig Init Fehler: " + ex.Message); }
            }
        }
            ...
            lock (_lock)
            {
                ...
                try
                {
                    ...
                    if (string.IsNullOrWhiteSpace(typeStr) || typeStr.Equals("None", StringComparison.OrdinalIgnoreCase))
                    {
                        ...
                        {
                            shouldLog = true; _loggedNoTypeOnce = true;
                        }
                    ...
                    if (int.TryParse(addrStr, out var addr) && addr > 0) inst.SspAddress = addr; else Log("INFO: Keine/ung�ltige SSPAddress � Standard wird verwendet.");
                    if (inst is SmartCoinV1 sc1) sc1.StatusChanged += OnStatusChanged;
                    try { inst.Connect(); Log("Connect() auf Ger�t 2 initiiert."); } catch (Exception ex) { Log("Connect-Fehler initial: " + ex.Message); }
                    Instance = inst;
                    _disabledByConfig = false; // active now
                    StartWatchdog();
                }
                catch (Exception ex) { Log("InitFromIni Fehler: " + ex.Message); }
                finally { _initialized = Instance != null; }
            }
        }
        ...
        private static void CancelActiveReconnect()
        {
            ...
            _reconnectTask = null; _reconnectCts = null;
            System.Threading.Interlocked.Exchange(ref _pendingReconnect, 0);
        }
        ...
        private static void OnStatusChanged(string status)
        {
            ...
            {
                if (System.Threading.Interlocked.CompareExchange(ref _pendingReconnect, 1, 0) != 0)
                    return;
        ...
        public static void Shutdown()
        {
            lock (_lock)
            {
                ...
                if (inst != null)
                {
                    try { inst.StatusChanged -= OnStatusChanged; } catch { }
                    try { inst.Enable(false); } catch { }
                    try { inst.HardDisconnect(); } catch { }
                    try { inst.SetStatus("getrennt"); } catch { }
                }
                else
                {
                    try { Instance?.Disconnect(); } catch { }
