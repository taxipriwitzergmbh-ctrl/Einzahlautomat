...
namespace TaMi_Einzahlautomat {
    ...
    public class PersonalStatus {
        ...
        public DateTime? AustrittAm {
            get;
            set;
        }

        public int AppRights1 {
            get;
            set;
        }

        public int AppRights2 {
            get;
            set;
        }
        ...
        public bool IsAutomatAdmin {
            get {
                return (this.AppRights2 & (int)SuE.TaMi.AppRights2.AUTOMAT_ADMIN) == (int)SuE.TaMi.AppRights2.AUTOMAT_ADMIN;
            }
        }
    }
    ...
    public class DatabaseHelper : IDisposable {
        ...
        private struct PersonalCacheEntry {
        ...
        public static void SetPersonalCacheTtlSeconds(int s) {
            if (s > 0 && s <= 600) _personalCacheTtl = TimeSpan.FromSeconds(s);
        }
            ...
            using (var cmd = _connection.CreateCommand()) {
                ...
                using (var rdr = await cmd.ExecuteReaderAsync(CommandBehavior.SingleRow).ConfigureAwait(false)) {
                    ...
                    var info = new KassenbuchDeviceInfo {
                        DeviceID = rdr.IsDBNull(rdr.GetOrdinal("DeviceID")) ? deviceId : Convert.ToInt32(rdr["DeviceID"]), AutomatenName = rdr.IsDBNull(rdr.GetOrdinal("AutomatenName")) ? null : Convert.ToString(rdr["AutomatenName"]), Standort = rdr.IsDBNull(rdr.GetOrdinal("Standort")) ? null : Convert.ToString(rdr["Standort"]), Gesperrt = !rdr.IsDBNull(rdr.GetOrdinal("Gesperrt")) && Convert.ToBoolean(rdr["Gesperrt"]), AllowedManID = rdr.IsDBNull(rdr.GetOrdinal("AllowedManID")) ? (int?)null : Convert.ToInt32(rdr["AllowedManID"])
                    }
        ...
        public async Task<List<string > > GetActiveMitarbeiterNotizenAsync(int pid) {
            ...
            using (var cmd = _connection.CreateCommand()) {
                cmd .CommandText = @" SELECT [Text] FROM TNotizen WITH (NOLOCK) WHERE RelTyp = 12 AND (TRY_CONVERT(int, RelID) = @PID OR TRY_CONVERT(int, RelID) = -1) AND (GueltigBis IS NULL OR GueltigBis <= '19000101' OR GueltigBis >= SYSDATETIME()) AND (Flags IS NULL OR Flags = 0) -- Berücksichtige Flags ORDER BY Datum DESC, NotizID DESC; ";
        ...
        public async Task<bool > PersonalIdExistsAsync(int pid) {
            ...
            using (var cmd = _connection.CreateCommand()) {
                cmd .CommandText = "SELECT TOP 1 1 FROM TPersonal WITH (NOLOCK) WHERE PID = @PID";
                cmd .Parameters.AddWithValue("@PID", pid);
        ...
        public async System.Threading.Tasks.Task<System.Collections.Generic.List<NotizenDbDto > > GetNotizenToConfirmAsync(int pid) {
            ...
            using (var cmd = _connection.CreateCommand()) {
                ...
                cmd .CommandText = @" SELECT NotizID, [Text], TRY_CONVERT(int, RelID) AS RelID, ISNULL(Flags,0) AS Flags, GueltigBis FROM TNotizen WITH (NOLOCK) WHERE RelTyp = 12 AND TRY_CONVERT(int, RelID) = @PID AND ISNULL(Flags,0) = 0 AND (GueltigBis IS NULL OR GueltigBis <= '19000101') ORDER BY Datum DESC, NotizID DESC; ";
                cmd .Parameters.AddWithValue("@PID", pid);
                ...
                using (var rdr = await cmd.ExecuteReaderAsync().ConfigureAwait(false)) {
                    while (await rdr.ReadAsync().ConfigureAwait(false)) {
                        try {
                            ...
                            try {
                                it .NotizId = rdr.IsDBNull(0) ? 0 : Convert.ToInt32(rdr[0]);
                            }
                            ...
                            try {
                                it .Text = rdr.IsDBNull(1) ? string.Empty : Convert.ToString(rdr[1]);
                            }
                            ...
                            try {
                                it .RelId = rdr.IsDBNull(2) ? 0 : Convert.ToInt32(rdr[2]);
                            }
                            ...
                            try {
                                it .Flags = rdr.IsDBNull(3) ? 0 : Convert.ToInt32(rdr[3]);
                            }
                            ...
                            try {
                                it .GueltigBis = rdr.IsDBNull(4) ? (DateTime?)null : Convert.ToDateTime(rdr[4]);
                            }
        ...
        public async Task<(bool exists, PersonalStatus status, string fahrercode) > GetPersonalLoginMetaAsync(int pid) {
            ...
            // Cache-Hit?
            if (_personalCache.TryGetValue(pid, out var entry) && DateTime.UtcNow < entry.ExpiryUtc) return (true, entry.Status, entry.Code);
            await EnsureOpenAsync().ConfigureAwait(false);
            ...
            using (var cmd = _connection.CreateCommand()) {
                ...
                using (var rdr = await cmd.ExecuteReaderAsync(System.Data.CommandBehavior.SingleRow).ConfigureAwait(false)) {
                    if (!await rdr.ReadAsync().ConfigureAwait(false)) return (false, null, null);
        ...
        public async Task<PersonalStatus > GetPersonalStatusAsync(int pid) {
            var meta = await GetPersonalLoginMetaAsync(pid).ConfigureAwait(false);
            return meta.exists ? meta.status : null;
        }

        private static void ParseAppRechteToUints(string appRechte, out uint u1, out uint u2) {
            u1 = 0u;
            u2 = 0u;
            if (string.IsNullOrEmpty(appRechte)) return;
            ...
            using (var cmd = _connection.CreateCommand()) {
                ...
                using (var reader = await cmd.ExecuteReaderAsync(CommandBehavior.SingleRow).ConfigureAwait(false)) {
                    ...
                    var details = new ShiftDetails {
                        SchichtId = reader.GetInt32(reader.GetOrdinal("SchichtId")), PersId = reader.GetInt32(reader.GetOrdinal("PersId")), PersName = reader.GetString(reader.GetOrdinal("PersName")), FhzId = reader.GetInt32(reader.GetOrdinal("FhzId")), ManId = Convert.ToInt32(reader["ManID"]), StartZeit = reader.GetDateTime(reader.GetOrdinal("StartZeit")), Betrag19 = reader.GetDecimal(reader.GetOrdinal("Betrag19")), Betrag7 = reader.GetDecimal(reader.GetOrdinal("Betrag7")), Betrag0 = reader.GetDecimal(reader.GetOrdinal("Betrag0")), EinzahlungBisher = reader.GetDecimal(reader.GetOrdinal("EinzahlungBisher"))
                    }

                    ;
                    // Falls ShiftDetails eine EndZeit-Property besitzt, setze sie (ohne API zu ändern)
                    try {
                        int ord = reader.GetOrdinal("EndZeit");

                        if (ord >= 0 && !reader.IsDBNull(ord)) {
                            var p = details.GetType().GetProperty("EndZeit");
                            if (p != null && p.CanWrite) p.SetValue(details, reader.GetDateTime(ord));
                        }
                    }
        ...
        // TKassenbuch: Eintrag erzeugen inkl. Kassenbestand-Fortschreibung (pro ManID+DeviceID)
        public async Task<int > InsertKassenbuchAsync(KassenbuchEntry entry) {
            ...
            if (entry.FhzId == 0 && entry.SchichtId > 0) {
                try {
                    using (var cmdFhz = _connection.CreateCommand()) {
                        cmdFhz .CommandText = "SELECT FhzId FROM TSchichten WITH (NOLOCK) WHERE SchichtId=@SID";
                        cmdFhz .Parameters.AddWithValue("@SID", entry.SchichtId);
                        var o = await cmdFhz.ExecuteScalarAsync().ConfigureAwait(false);
                        if (o != null && o != DBNull.Value && int.TryParse(o.ToString(), out var fhz)) entry.FhzId = fhz;
                    }
                }
            ...
            var typCode = MapTypStringToCode(entry.Typ);
            bool isPersonalguthaben = typCode == 5;
            decimal previousPgSaldo = 0m;
            ...
            if (isPersonalguthaben) {
                try {
                    ...
                    using (var cmdDup = _connection.CreateCommand()) {
                        cmdDup .CommandText = @"SELECT TOP 1 Belegnummer FROM TKassenbuch WITH (HOLDLOCK,UPDLOCK) WHERE Typ=5 AND DeviceID=@DID AND ManID=@MID AND PersId=@PID AND ISNULL(RevIsOld,0)=0 AND ISNULL(SchichtId,0)=@SID AND ISNULL(Betrag19,0)=@B19 AND ISNULL(Betrag7,0)=@B7 AND ISNULL(Betrag0,0)=@B0 AND ErfasstAm >= DATEADD(SECOND,-5, SYSDATETIME()) ORDER BY ErfasstAm DESC, Belegnummer DESC";
                        ...
                        if (existing != null && existing != DBNull.Value) {
                            try {
                                AppLogger .Log($"Personalguthaben-Duplikat verhindert (PID={entry.PersId}, Schicht={entry.SchichtId}, Δ={deltaPg:0.00} €) – Belegnummer {existing}");
                            }
            ...
            if (isPersonalguthaben && entry.PersId > 0) {
                try {
                    using (var cmdPrev = _connection.CreateCommand()) {
                        cmdPrev .CommandText = "SELECT TOP 1 SaldoPersonalguthaben FROM TKassenbuch WITH (NOLOCK) WHERE PersId=@pid AND Typ=5 AND DeviceID=@DID AND ISNULL(RevIsOld,0)=0 ORDER BY ErfasstAm DESC, Belegnummer DESC";
                        cmdPrev .Parameters.AddWithValue("@pid", entry.PersId);
                        cmdPrev .Parameters.AddWithValue("@DID", deviceId);
                        var o = await cmdPrev.ExecuteScalarAsync().ConfigureAwait(false);
                        if (o != null && o != DBNull.Value) previousPgSaldo = Convert.ToDecimal(o);
                    }
                }
            ...
            using (var cmdLast = _connection.CreateCommand()) {
                cmdLast .CommandText = "SELECT TOP 1 Kassenbestand FROM TKassenbuch WITH (HOLDLOCK,UPDLOCK) WHERE ManID=@MID AND DeviceID=@DID AND ISNULL(RevIsOld,0)=0 ORDER BY ErfasstAm DESC, Belegnummer DESC";
                cmdLast .Parameters.AddWithValue("@MID", entry.FirmenId);
                cmdLast .Parameters.AddWithValue("@DID", deviceId);
                var res = await cmdLast.ExecuteScalarAsync().ConfigureAwait(false);
                if (res != null && res != DBNull.Value) letzterKassenbestand = Convert.ToDecimal(res);
            }

            var delta = entry.BetragGesamt != 0m ? entry.BetragGesamt : (entry.Betrag19 + entry.Betrag7 + entry.Betrag0);
            var neuerKassenbestand = letzterKassenbestand + delta;
            if (neuerKassenbestand < 0m) throw new InvalidOperationException("NEGATIVE_KASSENBESTAND");
            int ? kassenBelegnummer = null;

            try {
                using (var cmdHas = _connection.CreateCommand()) {
                    cmdHas .CommandText = "SELECT 1 FROM sys.columns WHERE Name='KassenBelegnummer' AND Object_ID=OBJECT_ID('dbo.TKassenbuch')";
                    var has = await cmdHas.ExecuteScalarAsync().ConfigureAwait(false);

                    if (has != null && has != DBNull.Value) {
                        using (var cmdLastNo = _connection.CreateCommand()) {
                            cmdLastNo .CommandText = "SELECT TOP 1 ISNULL(KassenBelegnummer,0) FROM TKassenbuch WITH (NOLOCK) WHERE ManID=@MID AND DeviceID=@DID AND ISNULL(RevIsOld,0)=0 ORDER BY ErfasstAm DESC, Belegnummer DESC";
                            cmdLastNo .Parameters.AddWithValue("@MID", entry.FirmenId);
                            cmdLastNo .Parameters.AddWithValue("@DID", deviceId);
                            var o = await cmdLastNo.ExecuteScalarAsync().ConfigureAwait(false);
                            int last = (o != null && o != DBNull.Value) ? Convert.ToInt32(o) : 0;
                            kassenBelegnummer = last + 1;
                        }
                    }
                }
            }
            ...
            using (var cmd = _connection.CreateCommand()) {
                if (kassenBelegnummer.HasValue) cmd.CommandText = "INSERT INTO TKassenbuch (Typ,Buchungstext,Betrag19,Betrag7,Betrag0,Kassenbestand,Kost1,Kost2,Konto,PersId,SchichtId,ErfasstAm,SaldoPersonalguthaben,ManID,DeviceID,FhzId,KassenBelegnummer) VALUES (@Typ,@Buchungstext,@Betrag19,@Betrag7,@Betrag0,@Kassenbestand,@Kost1,@Kost2,@Konto,@PersId,@SchichtId,SYSDATETIME(),@SaldoPG,@ManID,@DeviceID,@FhzId,@KassenBelegnummer); SELECT SCOPE_IDENTITY();";
                else cmd.CommandText = "INSERT INTO TKassenbuch (Typ,Buchungstext,Betrag19,Betrag7,Betrag0,Kassenbestand,Kost1,Kost2,Konto,PersId,SchichtId,ErfasstAm,SaldoPersonalguthaben,ManID,DeviceID,FhzId) VALUES (@Typ,@Buchungstext,@Betrag19,@Betrag7,@Betrag0,@Kassenbestand,@Kost1,@Kost2,@Konto,@PersId,@SchichtId,SYSDATETIME(),@SaldoPG,@ManID,@DeviceID,@FhzId); SELECT SCOPE_IDENTITY();";
                cmd .Parameters.Add("@Typ", SqlDbType.TinyInt).Value = typCode;
                cmd .Parameters.AddWithValue("@Buchungstext", (object)entry.Buchungstext ?? DBNull.Value);
                cmd .Parameters.AddWithValue("@Betrag19", entry.Betrag19);
                cmd .Parameters.AddWithValue("@Betrag7", entry.Betrag7);
                cmd .Parameters.AddWithValue("@Betrag0", entry.Betrag0);
                cmd .Parameters.AddWithValue("@Kassenbestand", neuerKassenbestand);
                cmd .Parameters.AddWithValue("@Kost1", entry.Kost1);
                cmd .Parameters.AddWithValue("@Kost2", entry.Kost2);
                cmd .Parameters.AddWithValue("@Konto", entry.Konto);
                cmd .Parameters.AddWithValue("@PersId", entry.PersId);
                cmd .Parameters.AddWithValue("@SchichtId", entry.SchichtId);
                cmd .Parameters.AddWithValue("@SaldoPG", entry.SaldoPersonalguthaben);
                cmd .Parameters.AddWithValue("@ManID", entry.FirmenId);
                cmd .Parameters.AddWithValue("@DeviceID", deviceId);
                cmd .Parameters.AddWithValue("@FhzId", entry.FhzId);
                if (kassenBelegnummer.HasValue) cmd.Parameters.AddWithValue("@KassenBelegnummer", kassenBelegnummer.Value);
                var newIdObj = await cmd.ExecuteScalarAsync().ConfigureAwait(false);
                int newId = Convert.ToInt32(Convert.ToDecimal(newIdObj));

                if (isPersonalguthaben) {
                    try {
                        decimal betragDelta = entry.Betrag19 + entry.Betrag7 + entry.Betrag0;
                        decimal newSaldo = (entry.SaldoPersonalguthaben != 0m) ? entry.SaldoPersonalguthaben : previousPgSaldo + betragDelta;
                        decimal diff = newSaldo - previousPgSaldo;
                        AppLogger .Log($"Personalguthaben Änderung PID={entry.PersId}: {diff:+0.00;-0.00;0.00} € -> Saldo neu {newSaldo:0.00} €");
                    }
        ...
        public async Task<int > UpdateSchichtEinzahlungAsync(int schichtId, int persId, decimal sessionEingezahlt, decimal betrag19, decimal betrag7, decimal betrag0) {
            ...
            using (var cmd = _connection.CreateCommand()) {
                cmd .CommandText = "UPDATE TSchichten SET EinzahlungFahrer1=ISNULL(EinzahlungFahrer1,0)+@Betrag19,EinzahlungFahrer2=ISNULL(EinzahlungFahrer2,0)+@Betrag7,EinzahlungFahrer3=ISNULL(EinzahlungFahrer3,0)+@Betrag0,EinzahlungFahrer=ISNULL(EinzahlungFahrer1,0)+@Betrag19+ISNULL(EinzahlungFahrer2,0)+@Betrag7+ISNULL(EinzahlungFahrer3,0)+@Betrag0,Flags=Flags|1,AusgebuchtZeit=SYSDATETIME(),AusgebuchtPersId=@PersId WHERE SchichtId=@SchichtId";
                cmd .Parameters.AddWithValue("@SessionEingezahlt", sessionEingezahlt);
                cmd .Parameters.AddWithValue("@Betrag19", betrag19);
                cmd .Parameters.AddWithValue("@Betrag7", betrag7);
                cmd .Parameters.AddWithValue("@Betrag0", betrag0);
                cmd .Parameters.AddWithValue("@PersId", persId);
                cmd .Parameters.AddWithValue("@SchichtId", schichtId);
                return await cmd.ExecuteNonQueryAsync().ConfigureAwait(false);
            }
        ...
        public async Task<decimal > GetLastPersonalGuthabenSaldoAsync(int persId) {
            ...
            using (var cmd = _connection.CreateCommand()) {
                cmd .CommandText = "SELECT TOP 1 SaldoPersonalguthaben FROM TKassenbuch WITH (NOLOCK) WHERE PersId=@PersId AND Typ=5 AND DeviceID=@DID ORDER BY ErfasstAm DESC";
                cmd .Parameters.AddWithValue("@PersId", persId);
        ...
        public async Task<FahrzeugInfo > GetFahrzeugInfoAsync(int fid) {
            ...
            using (var cmd = _connection.CreateCommand()) {
                cmd .CommandText = "SELECT FID,Kennzeichen FROM TFahrzeuge WHERE FID=@FID";
                cmd .Parameters.AddWithValue("@FID", fid);

                using (var reader = await cmd.ExecuteReaderAsync().ConfigureAwait(false)) {
                    if (await reader.ReadAsync().ConfigureAwait(false)) return new FahrzeugInfo {
                        FID = reader.GetInt32(reader.GetOrdinal("FID")), Kennzeichen = reader["Kennzeichen"] as string ?? ""
                    }
        ...
        public async Task<DataTable > GetShiftSelectionTableAsync(int persId) {
            ...
            using (var cmd = _connection.CreateCommand()) {
                cmd .CommandText = "SELECT s.SchichtId,s.StartZeit,s.FhzId,f.Kennzeichen,CAST(ISNULL(s.EinnahmenBar1,0)+ISNULL(s.EinnahmenBar2,0)+ISNULL(s.EinnahmenBar3,0) AS money) AS EinnahmenBar FROM TSchichten s LEFT JOIN TFahrzeuge f ON s.FhzId=f.FID WHERE s.Flags=0 AND s.PersId=@PersId ORDER BY s.StartZeit DESC";
                cmd .Parameters.AddWithValue("@PersId", persId);
        ...
        public async Task<DataTable > GetOffeneAuszahlungenAsync(int persId) {
            ...
            using (var cmd = _connection.CreateCommand()) {
                cmd .CommandText = "SELECT Belegnummer,CASE CAST(Typ AS int) WHEN 1 THEN 'Anfangsbestand' WHEN 2 THEN 'Einzahlung' WHEN 3 THEN 'Auszahlung' WHEN 4 THEN 'Schichtabrechnung' WHEN 5 THEN 'Personalguthaben' WHEN 6 THEN 'Trinkgeld' ELSE '' END AS Typ,Buchungstext,Betrag19,Betrag7,Betrag0,CASE WHEN COLUMNPROPERTY(OBJECT_ID('dbo.TKassenbuchZahlungen'),'BetragGesamt','ColumnId') IS NOT NULL AND BetragGesamt IS NOT NULL THEN BetragGesamt ELSE ISNULL(Betrag19,0)+ISNULL(Betrag7,0)+ISNULL(Betrag0,0) END AS BetragGesamt,PersId,ErfasstAm,ManID AS FirmenID,Kost1,Kost2,Konto,DeviceID,Verbucht FROM TKassenbuchZahlungen WITH (NOLOCK) WHERE PersId=@pid AND (Verbucht=0 OR Verbucht IS NULL)";
        ...
        public async Task<DataTable > GetMandantenAsync(bool onlyForPayments = false) {
            ...
            if (onlyForPayments) {
                try {
                    var candidates = new[] {
                        "GeldautomatAktiv","KassenClientAktiv","KassenclientAktiv","ZahlungAktiv","ZahlungenAktiv","EnableZahlung","IsActive","Aktiv"
                    }

                    ;

                    using (var chk = _connection.CreateCommand()) {
                        chk .CommandText = "SELECT name FROM sys.columns WHERE object_id=OBJECT_ID('dbo.TMandanten')";
                        var names = new HashSet<string>(StringComparer.OrdinalIgnoreCase);

                        using (var rdr = await chk.ExecuteReaderAsync().ConfigureAwait(false)) {
                            while (await rdr.ReadAsync().ConfigureAwait(false)) {
                                string n = rdr[0] as string; if (!string.IsNullOrEmpty(n)) names.Add(n);
                            }
                        }

                        foreach (var c in candidates) if (names.Contains(c)) {
                            filterColumn = c;
                            break;
                        }
                    }
            ...
            using (var cmd = _connection.CreateCommand()) {
                if (onlyForPayments) {
                    var where = "WHERE (ISNULL(Flags,0) & 512)=0";
                    if (!string.IsNullOrEmpty(filterColumn)) where += $" AND ISNULL([{filterColumn}],0)<>0";
                    cmd .CommandText = $"SELECT ManID,ManName FROM TMandanten WITH (NOLOCK) {where} ORDER BY ManName";
                }
        ...
        public async Task<int > InsertKassenbuchZahlungAsync(byte typ,string buchungstext,decimal betrag19,decimal betrag7,decimal betrag0,int persId,int firmenId,int? kost1,int? kost2,int? konto,decimal betragGesamt) {
            ...
            try {
                using (var chk=_connection.CreateCommand()) {
                    chk .CommandText="SELECT 1 FROM sys.columns WHERE Name='UserAnlage' AND Object_ID=OBJECT_ID('dbo.TKassenbuchZahlungen')";
                    var o= await chk.ExecuteScalarAsync().ConfigureAwait(false);
                    hasUserAnlage =o !=null && o!=DBNull.Value;
                }
            }
            ...
            using (var cmd=_connection.CreateCommand()) {
                cmd .CommandText = hasUserAnlage? "INSERT INTO TKassenbuchZahlungen (Typ,Buchungstext,Betrag19,Betrag7,Betrag0,PersId,ManID,DeviceID,ErfasstAm,Kost1,Kost2,Konto,BetragGesamt,Verbucht,UserAnlage) VALUES (@Typ,@Buchungstext,@Betrag19,@Betrag7,@Betrag0,@PersId,@ManID,@DeviceID,SYSDATETIME(),@Kost1,@Kost2,@Konto,@BetragGesamt,0,@UserAnlage); SELECT SCOPE_IDENTITY();" : "INSERT INTO TKassenbuchZahlungen (Typ,Buchungstext,Betrag19,Betrag7,Betrag0,PersId,ManID,DeviceID,ErfasstAm,Kost1,Kost2,Konto,BetragGesamt,Verbucht) VALUES (@Typ,@Buchungstext,@Betrag19,@Betrag7,@Betrag0,@PersId,@ManID,@DeviceID,SYSDATETIME(),@Kost1,@Kost2,@Konto,@BetragGesamt,0); SELECT SCOPE_IDENTITY();";
                cmd .Parameters.AddWithValue("@Typ", typ);
                cmd .Parameters.AddWithValue("@Buchungstext", (object)buchungstext ?? DBNull.Value);
                cmd .Parameters.AddWithValue("@Betrag19", betrag19);
                cmd .Parameters.AddWithValue("@Betrag7", betrag7);
                cmd .Parameters.AddWithValue("@Betrag0", betrag0);
                cmd .Parameters.AddWithValue("@PersId", persId);
                cmd .Parameters.AddWithValue("@ManID", firmenId);
                cmd .Parameters.AddWithValue("@DeviceID", CurrentDeviceId);
                cmd .Parameters.AddWithValue("@Kost1", (object)kost1 ?? DBNull.Value);
                cmd .Parameters.AddWithValue("@Kost2", (object)kost2 ?? DBNull.Value);
                cmd .Parameters.AddWithValue("@Konto", (object)konto ?? DBNull.Value);
                cmd .Parameters.AddWithValue("@BetragGesamt", betragGesamt);
                if (hasUserAnlage) cmd.Parameters.AddWithValue("@UserAnlage", persId);
                var o2= await cmd.ExecuteScalarAsync().ConfigureAwait(false);
                return Convert.ToInt32(Convert.ToDecimal(o2));
            }
        }
        ...
        public async Task<DataTable > GetZeiterfassungAsync(int pid, DateTime von, DateTime bis) {
            ...
            using (var cmd = _connection.CreateCommand()) {
                cmd .CommandText = @"SELECT h.AutoID, h.Typ, h.ZeitVon, h.ZeitBis, h.PID, h.FID, h.Bemerkung, h.ZeitAnlage, h.UserAnlage, h.ZeitBearbeitet, h.UserBearbeitet, f.Kennzeichen FROM THistoryZeiterfassung h WITH (NOLOCK) LEFT JOIN TFahrzeuge f ON h.FID = f.FID WHERE h.PID = @PID AND h.ZeitVon >= @Von AND h.ZeitVon < @Bis ORDER BY h.ZeitVon";
                cmd .Parameters.AddWithValue("@PID", pid);
        ...
        // NEW: Zeiterfassung stempeln
        public async Task<int? > GetRecentOpenWorkEntryIdAsync(int pid, int hours) {
            await EnsureOpenAsync().ConfigureAwait(false);

            using (var cmd = _connection.CreateCommand()) {
                cmd .CommandText = @"SELECT TOP 1 AutoID FROM THistoryZeiterfassung WITH (NOLOCK) WHERE PID=@PID AND Typ=1 AND ZeitVon >= DATEADD(HOUR, -@HOURS, SYSDATETIME()) AND (ZeitBis IS NULL OR ZeitBis <= '19000101') ORDER BY ZeitVon DESC";
                cmd .Parameters.AddWithValue("@PID", pid);
        ...
        public async Task<int > InsertWorkStartAsync(int pid) {
            await EnsureOpenAsync().ConfigureAwait(false);

            using (var cmd = _connection.CreateCommand()) {
                cmd .CommandText = @"INSERT INTO THistoryZeiterfassung (Typ,ZeitVon,ZeitBis,PID,FID,Bemerkung,ZeitAnlage,UserAnlage,ZeitBearbeitet,UserBearbeitet) VALUES (1,SYSDATETIME(),'19000101',@PID,0,'Automat',SYSDATETIME(),0,SYSDATETIME(),0); SELECT SCOPE_IDENTITY(); ";
                cmd .Parameters.AddWithValue("@PID", pid);
                var o = await cmd.ExecuteScalarAsync().ConfigureAwait(false);
        ...
        public async Task<int > UpdateWorkStopAsync(int autoId) {
            await EnsureOpenAsync().ConfigureAwait(false);
        ...
        public async Task<int? > GetRecentOpenPauseEntryIdAsync(int pid, int hours) {
            ...
            using (var cmd = _connection.CreateCommand()) {
                cmd .CommandText = @"SELECT TOP 1 AutoID FROM THistoryZeiterfassung WITH (NOLOCK) WHERE PID=@PID AND Typ=106 AND ZeitVon >= DATEADD(HOUR, -@HOURS, SYSDATETIME()) AND (ZeitBis IS NULL OR ZeitBis <= '19000101') ORDER BY ZeitVon DESC";
                cmd .Parameters.AddWithValue("@PID", pid);
                cmd .Parameters.AddWithValue("@HOURS", hours);
                var o = await cmd.ExecuteScalarAsync().ConfigureAwait(false);
                if (o == null || o == DBNull.Value) return null;
        ...
        public async Task<int > InsertPauseStartAsync(int pid) {
            ...
            using (var cmd = _connection.CreateCommand()) {
                cmd .CommandText = @"INSERT INTO THistoryZeiterfassung (Typ,ZeitVon,ZeitBis,PID,FID,Bemerkung,ZeitAnlage,UserAnlage,ZeitBearbeitet,UserBearbeitet) VALUES (106,SYSDATETIME(),'19000101',@PID,0,'Automat',SYSDATETIME(),0,SYSDATETIME(),0); SELECT SCOPE_IDENTITY(); ";
                cmd .Parameters.AddWithValue("@PID", pid);
                var o = await cmd.ExecuteScalarAsync().ConfigureAwait(false);
                return Convert.ToInt32(Convert.ToDecimal(o));
            }
        ...
        public async Task MarkZahlungAlsVerbuchtAsync(int belegnummer) {
            ...
            using (var cmd=_connection.CreateCommand()) {
                cmd .CommandText="UPDATE TKassenbuchZahlungen SET Verbucht=1, DeviceID=@DID WHERE Belegnummer=@bnr";
                cmd .Parameters.AddWithValue("@DID", CurrentDeviceId);
        ...
        public async Task<string > GetFahrercodeAsync(int pid) {
            var meta = await GetPersonalLoginMetaAsync(pid).ConfigureAwait(false);
            return meta.exists ? meta.fahrercode : null;
        }
        ...
        public async Task<PersonalInfo > GetPersonalByNfcAsync(string nfcToken) {
            if (string.IsNullOrWhiteSpace(nfcToken)) return null;
            await EnsureOpenAsync().ConfigureAwait(false);
        ...
        public async Task<int > SetNfcAsync(int pid,string nfc) {
            ...
            using (var cmd=_connection.CreateCommand()) {
                cmd .CommandText="UPDATE TPersonal SET NFCTagUID=@NFC WHERE PID=@PID";
                cmd .Parameters.AddWithValue("@NFC", (object)nfc ?? DBNull.Value);
                cmd .Parameters.AddWithValue("@PID", pid);
                int rows= await cmd.ExecuteNonQueryAsync().ConfigureAwait(false);
        ...
        private static byte MapTypStringToCode(string typ) {
            if (string.IsNullOrWhiteSpace(typ)) return 0;
            ...
            switch (typ.Trim().ToLowerInvariant()) {
                ...
                case "trinkgeld": case "trinkgeld auszahlung": return 6;
                default: return 0;
            }
        }
        ...
        public class DienstkontoInfo {
            ...
            public override string ToString() {
                return Name ?? (Absender ?? Benutzername ?? (Host ?? ""));
            }
        }
                ...
                using (var rdr = await cmd.ExecuteReaderAsync().ConfigureAwait(false)) {
                    while (await rdr.ReadAsync().ConfigureAwait(false)) {
                        var info = new DienstkontoInfo {
                            ID = rdr.IsDBNull(rdr.GetOrdinal("ID")) ? 0 : Convert.ToInt32(rdr["ID"]), Typ = rdr.IsDBNull(rdr.GetOrdinal("Typ")) ? (byte?)null : Convert.ToByte(rdr["Typ"]), Name = rdr.IsDBNull(rdr.GetOrdinal("Name")) ? null : Convert.ToString(rdr["Name"]), Host = rdr.IsDBNull(rdr.GetOrdinal("Host")) ? null : Convert.ToString(rdr["Host"]), Port = rdr.IsDBNull(rdr.GetOrdinal("Port")) ? null : Convert.ToString(rdr["Port"]), Benutzername = rdr.IsDBNull(rdr.GetOrdinal("Benutzername")) ? null : Convert.ToString(rdr["Benutzername"]), Passwort = rdr.IsDBNull(rdr.GetOrdinal("Passwort")) ? null : Convert.ToString(rdr["Passwort"]), Absender = rdr.IsDBNull(rdr.GetOrdinal("Absender")) ? null : Convert.ToString(rdr["Absender"]),
                        }
        ...
        public const byte TAMI_EVENT_EZATM_LOGIN_PERSID = 91;
        public const byte TAMI_EVENT_EZATM_LOGIN_NFCTAG = 92;

        public async Task LogEzAtmLoginHistoryAsync(int persId, byte eventTyp,string automatName) {
            if (persId <= 0) return;
            string eventText;

            switch (eventTyp) {
                case TAMI_EVENT_EZATM_LOGIN_PERSID: eventText = $"Anmeldung Einzahlautomat {automatName} mit Pin und Passwort";
                break;
                case TAMI_EVENT_EZATM_LOGIN_NFCTAG: eventText = $"Anmeldung Einzahlautomat {automatName} mit NFC";
                break;
                default: throw new ArgumentOutOfRangeException(nameof(eventTyp));
            }
