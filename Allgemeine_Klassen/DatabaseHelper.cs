    ...
    public class DatabaseHelper : IDisposable
    {
        ...
        public async Task<int> InsertKassenbuchAsync(KassenbuchEntry entry)
        {
            ...
            using (var cmdLast = _connection.CreateCommand())
            {
                ...
                cmdLast.Parameters.AddWithValue("@MID", entry.FirmenId); cmdLast.Parameters.AddWithValue("@DID", deviceId);
        ...
        { await EnsureOpenAsync().ConfigureAwait(false); bool hasUserAnlage=false; try { using (var chk=_connection.CreateCommand()) { chk.CommandText="SELECT 1 FROM sys.columns WHERE Name='UserAnlage' AND Object_ID=OBJECT_ID('dbo.TKassenbuchZahlungen')"; var o= await chk.ExecuteScalarAsync().ConfigureAwait(false); hasUserAnlage=o!=null && o!=DBNull.Value; } } catch { hasUserAnlage=false; } using (var cmd=_connection.CreateCommand()) { cmd.CommandText = hasUserAnlage? "INSERT INTO TKassenbuchZahlungen (Typ,Buchungstext,Betrag19,Betrag7,Betrag0,PersId,ManID,DeviceID,ErfasstAm,Kost1,Kost2,Konto,BetragGesamt,Verbucht,UserAnlage) VALUES (@Typ,@Buchungstext,@Betrag19,@Betrag7,@Betrag0,@PersId,@ManID,@DeviceID,SYSDATETIME(),@Kost1,@Kost2,@Konto,@BetragGesamt,0,@UserAnlage); SELECT SCOPE_IDENTITY();" : "INSERT INTO TKassenbuchZahlungen (Typ,Buchungstext,Betrag19,Betrag7,Betrag0,PersId,ManID,DeviceID,ErfasstAm,Kost1,Kost2,Konto,BetragGesamt,Verbucht) VALUES (@Typ,@Buchungstext,@Betrag19,@Betrag7,@Betrag0,@PersId,@ManID,@DeviceID,SYSDATETIME(),@Kost1,@Kost2,@Konto,@BetragGesamt,0); SELECT SCOPE_IDENTITY();"; cmd.Parameters.AddWithValue("@Typ", typ); cmd.Parameters.AddWithValue("@Buchungstext", (object)buchungstext ?? DBNull.Value); cmd.Parameters.AddWithValue("@Betrag19", betrag19); cmd.Parameters.AddWithValue("@Betrag7", betrag7); cmd.Parameters.AddWithValue("@Betrag0", betrag0); cmd.Parameters.AddWithValue("@PersId", persId); cmd.Parameters.AddWithValue("@ManID", firmenId); cmd.Parameters.AddWithValue("@DeviceID", CurrentDeviceId); cmd.Parameters.AddWithValue("@Kost1", (object)kost1 ?? DBNull.Value); cmd.Parameters.AddWithValue("@Kost2", (object)kost2 ?? DBNull.Value); cmd.Parameters.AddWithValue("@Konto", (object)konto ?? DBNull.Value); cmd.Parameters.AddWithValue("@BetragGesamt", betragGesamt); if (hasUserAnlage) cmd.Parameters.AddWithValue("@UserAnlage", persId); var o2= await cmd.ExecuteScalarAsync().ConfigureAwait(false); return Convert.ToInt32(Convert.ToDecimal(o2)); } }
        ...
        public async Task<int?> GetRecentOpenWorkEntryIdAsync(int pid, int hours)
        {
            ...
            using (var cmd = _connection.CreateCommand())
            {
                cmd.CommandText = @"SELECT TOP 1 AutoID FROM THistoryZeiterfassung WITH (NOLOCK)
                                    WHERE PID=@PID AND Typ=1 AND ZeitVon >= DATEADD(HOUR, -@HOURS, SYSDATETIME())
        ...
        public async Task<int> InsertWorkStartAsync(int pid)
        {
            ...
            using (var cmd = _connection.CreateCommand())
            {
                cmd.CommandText = @"INSERT INTO THistoryZeiterfassung (Typ,ZeitVon,ZeitBis,PID,FID,Bemerkung,ZeitAnlage,UserAnlage,ZeitBearbeitet,UserBearbeitet)
                                    VALUES (1,SYSDATETIME(),'19000101',@PID,0,'Automat',SYSDATETIME(),0,SYSDATETIME(),0);
                                    SELECT SCOPE_IDENTITY();";
                cmd.Parameters.AddWithValue("@PID", pid);
                var o = await cmd.ExecuteScalarAsync().ConfigureAwait(false);
